...
public class StudentService {
    public DefaultTableModel myRegistrations(int studentUserId) throws SQLException {
        String[] cols = {"Enrollment ID","Section ID","Code","Title","Day/Time","Room","Status"};
        DefaultTableModel m = new DefaultTableModel(cols,0){
            @Override
            public boolean isCellEditable(int r,int c){return false;}
        };
        String sql = "SELECT e.enrollment_id, s.section_id, c.code, c.title, s.day_time, s.room, e.status " +
        ...
        try (Connection c = ErpDb.get()) {
            ...
            try (PreparedStatement psFull = c.prepareStatement(checkFull);
                ...
                try (ResultSet rs = psFull.executeQuery()) {
                    if (!rs.next()) throw new RuntimeException("Section not found.");
                    if (rs.getInt(1) <= 0) throw new RuntimeException("Section full.");
                }
                psDup.setInt(1, s.userId);
                psDup.setInt(2, sectionId);
                try (ResultSet rs2 = psDup.executeQuery()) {
                    if (rs2.next()) throw new RuntimeException("Already registered for this section.");
                }
                try (PreparedStatement ins = c.prepareStatement(
                        "INSERT INTO enrollments(student_user_id, section_id, status) VALUES(?,?, 'ENROLLED')")) {
                    ins.setInt(1, s.userId); ins.setInt(2, sectionId); ins.executeUpdate();
                }
                c.commit();
    ...
    public void drop(Session s, int enrollmentId) throws SQLException {
        ...
        if (LocalDate.now().isAfter(ddl)) throw new RuntimeException("Drop deadline passed.");
        String sql = "DELETE FROM enrollments WHERE enrollment_id=? AND student_user_id=?";
        try (Connection c = ErpDb.get(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, enrollmentId); ps.setInt(2, s.userId);
            int n = ps.executeUpdate();
            if (n == 0) throw new RuntimeException("Not found or not your enrollment.");
        }
    }
    ...
    public DefaultTableModel grades(int studentUserId) throws SQLException {
        ...
        try (Connection c = ErpDb.get(); PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    m.addRow(new Object[]{rs.getString(1), rs.getInt(2), rs.getString(3), rs.getBigDecimal(4), rs.getBigDecimal(5)});
                }
            }
        }
        return m;
